...
# multilib
Source4:        luaconf.h
Patch0:         %{system_name}-5.4.0-beta-autotoolize.patch
Patch1:         %{system_name}-5.3.0-idsize.patch
#Patch2:         %%{system_name}-5.3.0-luac-shared-link-fix.patch
Patch3:         %{system_name}-5.2.2-configure-linux.patch
Patch4:         %{system_name}-5.3.0-configure-compat-module.patch
%if 0%{?bootstrap}
Patch5:         %{system_name}-5.3.0-autotoolize.patch
Patch6:		%{system_name}-5.3.5-luac-shared-link-fix.patch
%endif
# https://www.lua.org/bugs.html
Patch18:	%{system_name}-5.3.5-CVE-2020-24370.patch
Patch19:	%{system_name}-5.4.2-CVE-2022-33099.patch
Patch20:	%{system_name}-5.4-CVE-2022-28805.patch

BuildRequires:  automake autoconf libtool EDOreadline-devel EDOncurses-devel
BuildRequires: make
...
%package devel
Summary:        Development files for %{system_name}
...
Requires:       EDOpkg-config

%description devel
This package contains development files for %{system_name}.
...
%description libs
This package contains the shared libraries for %{system_name}.

%package static
Summary:        Static library for %{system_name}
Requires:       %{name}%{?_isa} = %{version}-%{release}

%description static
This package contains the static version of liblua for %{system_name}.


%prep
%if 0%{?bootstrap}
%setup -q -a 2 -a 3 -n %{system_name}-%{version}
%else
%setup -q -a 3
%endif
cp %{SOURCE1} .
